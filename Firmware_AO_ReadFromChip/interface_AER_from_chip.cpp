...
void AER_from_chip::recordEvent() volatile {
  uint8_t interface;
    ...
    uint32_t address = getData();
    send_data32(interface, address);

    // Extract neuron address (adjust depending on chip's encoding)
    uint8_t neuron = (uint8_t)(address & 0xFF);

    // Trigger recurrency logic
    recurrency_check_and_forward(neuron);
}
